      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-3 text-zinc-600">
            ...
            <button 
              onClick={(e) => { e.stopPropagation(); onClose(); setIsMinimized(false); }} 
        ...
        <div className={`flex flex-col flex-1 bg-white ${isMinimized ? 'hidden' : 'flex'}`}>
          <div className="flex items-center border-b border-zinc-100 px-4 py-2">
            ...
            <input 
              ...
              type="email" 
              className="flex-1 outline-none text-zinc-800 text-sm py-1" 
            />
          </div>
          <div className="flex items-center border-b border-zinc-100 px-4 py-2">
            <input 
              ...
              type="text" 
              placeholder="Subject" 
              className="flex-1 outline-none text-zinc-800 text-sm py-1 placeholder:text-zinc-500" 
            />
          </div>
          <textarea 
            value={body}
            onChange={(e) => setBody(e.target.value)}
            className="flex-1 resize-none outline-none p-4 text-zinc-800 text-sm font-sans" 
          />
          <div className="p-3 flex items-center justify-between bg-white border-t border-zinc-100">
            <button 
              onClick={handleSend}
              className="bg-[#0B57D0] text-white px-6 py-2 rounded-full font-bold text-sm hover:bg-blue-700 hover:shadow transition-all"
            >
              Send
            </button>
          </div>
